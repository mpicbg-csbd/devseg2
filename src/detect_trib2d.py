...
def train(m,vd,td,ta):
  ...
  w = torch.zeros(1,1,13,128,128).cuda() ## just create once
  ...
  for ta.i in range(ta.i,ta.i_final):
    ...
    _pt = td.gt[sc][_ipt]
    # print(_pt.shape)
    # _pt = np.floor(np.random.rand(3)*(_sh-(13,128,128))).astype(int)
    _pt = _pt + (2*np.random.rand(3)-1)*(2,40,40) - (8,64,64) ## center patch and jitter
    _pt = _pt.clip(min=[0,0,0],max=[_sh-(13,128,128)])[0]
    _pt = _pt.astype(int)
    sz,sy,sx = slice(_pt[0],_pt[0]+13), slice(_pt[1],_pt[1]+128), slice(_pt[2],_pt[2]+128)
