...
def init(trainer):
  ...
  vd = trainer.load_data(ta.valitimes, trainer)
  td = trainer.load_data(ta.traintimes, trainer)
...
def train(T,trainer):
  ...
  for ta.i in range(ta.i,ta.i_final):
    ...
    x,yt,w = trainer.sampler(ta,td,trainer)
...
def load_isbi_training_data(times,trainer):
  d = SimpleNamespace()
